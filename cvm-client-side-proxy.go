...
func main() {
	...
	if *client {
		client_side_tls_termination(proxyTarget, strconv.Itoa(*listenPort))
	}

	if *server {
		server_side_tls_termination(proxyTarget, strconv.Itoa(*listenPort), *measurements)
	}
}

func client_side_tls_termination(targetUrl string, listenPort string) {
	issuer := tdx.NewIssuer(nil)
	tlsConfig, err := atls.CreateAttestationClientTLSConfig(issuer, nil)
	if err != nil {
...
func server_side_tls_termination(proxyTarget string, listenPort string, measurementsPath string) {
	...
	confTLS, err := atls.CreateAttestationServerTLSConfig(nil, validators)
	if err != nil {
		panic(err)
	}
